...
if [ -f "/etc/udev/rules.d/98-omo-r1.rules" ]; then
    ...
    echo 'SUBSYSTEM=="tty", KERNELS=="3-1", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", GROUP:="dialout", SYMLINK+="ttyMotor"' > /etc/udev/rules.d/98-omo-r1.rules
...
if [ -f "/etc/udev/rules.d/ydlidar.rules" ]; then
    ...
    echo 'SUBSYSTEM=="tty", KERNELS=="3-2", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", GROUP:="dialout",  SYMLINK+="ydlidar"' >/etc/udev/rules.d/97-ydlidar.rules
